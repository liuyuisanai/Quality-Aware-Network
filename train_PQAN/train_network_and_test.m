...
for split_index=param.use_data_split_index:param.data_split_num
   ...
   % net=caffe.get_net(param.test_net_file,'test');
    ...
    %
    batch_score=ones(1,1,3,batch_size,'single');
    %
    train_x_axis=[];
    train_y_axis=[];
    iter=0;
    while iter<param.train_maxiter
        [batch_data,batch_label]=get_train_minibatch(param,batch_data,batch_label,batch_size,train_data_cam1,train_data_cam2,label_train_cam1,label_train_cam2);
        ...
        if mod(iter,10)==0
            ...
            loss=caffe_solver.nets{1}.blobs('triplet_loss').get_data;
